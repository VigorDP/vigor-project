...
const RUBBERBAND_LINE_WIDTH = 1;
const RUBBERBAND_STROKE_STYLE = 'green';

let drawingSurfaceImageData; // 暂存 canvas 绘制数据
let dragging = false; // 是否正在拖动鼠标
const editingText = false; // 是否正在编辑文本
let editingCurve = false; // 是否正在编辑贝塞尔曲线
let draggingControlPoint = false; // 是否正在编辑贝塞尔曲线控制点
const mousedown = {
  x: 0,
  y: 0,
};
...
function drawBackground(context: any, color: any, stepx: any, stepy: any) {
  const backgroundContext = document.createElement('canvas').getContext('2d');
...
function listenEvent2(drawingContext, drawingCanvas) {
  drawingCanvas.onmousedown = function (e) {
    ...
    const loc = CommonCanvasHelpers.windowToCanvas(drawingCanvas, x, y);
    e.preventDefault();
...
function mouseMoveInDrawingCanvas(loc, drawingContext, drawingCanvas) {
  if (dragging) {
    ...
    if (
      IconCanvasUtils.selectedFunction === 'line' ||
...
function mouseUpInDrawingCanvas(loc, drawingContext) {
  ...
  if (dragging) {
    if (IconCanvasUtils.selectedFunction === 'erase') {
      ...
      if (IconCanvasUtils.selectedFunction === 'line') finishDrawingLine(loc, drawingContext);
      else if (IconCanvasUtils.selectedFunction === 'rectangle') finishDrawingRectangle(drawingContext);
      else if (IconCanvasUtils.selectedFunction === 'circle') finishDrawingCircle(loc, drawingContext);
...
function eraseLast(drawingContext) {
  let x = lastX - ERASER_RADIUS - ERASER_LINE_WIDTH,
    y = lastY - ERASER_RADIUS - ERASER_LINE_WIDTH,
    w = ERASER_RADIUS * 2 + ERASER_LINE_WIDTH * 2,
    h = w;
  const cw = drawingContext.canvas.width,
...
function drawRubberbandCircle(loc, drawingContext) {
  const angle = Math.atan(rubberbandH / rubberbandW);
  let radius = rubberbandH / Math.sin(angle);
